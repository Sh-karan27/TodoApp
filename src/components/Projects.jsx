      ...
      <div className='w-full h-full   flex flex-col items-center justify-start gap-5 rounded-xl '>
        <div className='w-full flex  items-center justify-between'>
          ...
          <div className='flex items-center justify-center gap-2'>
            ...
            <div className='bg-[#26304A] text-[#219ebc] px-2   rounded-full'>
              {data.length}
            </div>
          </div>
        </div>
        ...
        <div
          className={`w-full ${
            data.length === 0 ? '' : 'h-[150px]'
          }  flex flex-col items-center justify-start gap-5 overflow-y-scroll `}>
          {data.map((project, id) => {
            return (
              <div
                key={id}
                ...
                <div className='flex items-center justify-center gap-1'>
                  <IoIosTrash
                    className='text-red-500 text-2xl hover:cursor-pointer hover:text-blue-500'
                    onClick={() => deleteProjectText(project.id)}
                  />
                  <CiBookmarkCheck
                    className='text-blue-500 text-2xl hover:cursor-pointer hover:text-gray-500'
                    onClick={() => compeleteProject(project.id)}
                  />
                </div>
              </div>
            );
